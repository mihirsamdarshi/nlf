...
function parseInstalled(data, options, callback) {
    ...
    let count = 0
        ...
        (function parseModuleData(moduleData, currentDepth, output) {
            ...
            if (!Object.prototype.hasOwnProperty.call(output, moduleData.full)) {
...
function globIgnoringModules(directory, filePattern, callback) {
    ...
    const subdirs = glob.globSync('*/', {
        cwd: directory, ignore: ['**/node_modules/**', '**/bower_components/**']
    })
    ...
    const allDirs = glob.globSync(globpatterns, {
        nocase: true, cwd: directory, ignore: ['**/node_modules/**', '**/bower_components/**']
    })
...
function createModule(moduleData, callback) {
    const repository = (moduleData.repository || {}).url || moduleData.repository || '(none)'
    ...
    findPotentialLicenseFiles(directory, '*li@(c|s)en@(c|s)e*', function(err, licenseFiles) {
        if (err) {
            return callback(err)
        }
...
function find(options, callback) {
    ...
    const depOptions = {
        dev: true, extraFields: ['repository', 'licenses', 'license']
    }
...
module.exports = {
    find, Module, FileSource, PackageSource, csvFormatter, standardFormatter, licenseFind, LicenseCollection
}
